/*
* Clone of api_config.js with classic export
...
module.exports = {
    ...
    // config.get('device.id');        // returns a string
    // config.get('debug.level');      // returns an integer
    // config.get('wifi.sta.enable');  // returns a boolean
    // ```
    get: function(path) {
    ...
    // ```javascript
    // let config = require('config.js');
    // config.set({wifi: {ap: {enable: false}}});  // Disable WiFi AP mode
    // config.set({debug: {level: 3}});            // Set debug level to 3
    // ```
